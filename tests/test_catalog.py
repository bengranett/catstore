...
def test_query_box_scaling(nqueries=1000, nangles=13., n=10000):
    # import matplotlib
    ...
    for loop in range(4):
        ...
        imagecoord = np.random.uniform(0, 100, (n, 2))
        data = {'imagecoord': imagecoord}
        cat = catalogue.Catalogue(data, scale_x=3., scale_y=40., angle=30)
        ...
        for angle in np.linspace(0, 360, nangles):
            ...
            # height = np.ones(len(x))*3.0
            width = np.random.uniform(1,10,nqueries)
            height = np.random.uniform(1,10,nqueries)
